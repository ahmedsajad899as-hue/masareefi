...
async function _lvPickCamera(deviceId, label) {
  ...
  card.innerHTML = `<video id="${cam.id}-video" autoplay playsinline muted style="width:100%;max-width:200px;aspect-ratio:1/1;height:auto;object-fit:cover;display:block;margin:0 auto"></video>
...
function _lvUpdateCamGrid() {
  ...
  for (const c of _lvCams) {
    ...
    if (v) { v.style.height = 'auto'; v.style.aspectRatio = '1/1'; v.style.maxWidth = _lvCams.length === 1 ? '200px' : ''; }
  }
}
